...
class CharmDateWorkpage(LoginRequiredMixin,View):
    ...
    def post(self,request):
        ...
        table = CharmDateTable.objects.filter(id=int(table_id))
        new_data = TableData(
            data = float(value),
...
def get_id_list():
    ...
    data = (int(i.operator.pk) for i in CharmDateTable.objects.all())
...
def make_get():
    ...
    data = {f'table_{a.id}': {'table':
                 ...
                 for a in
            CharmDateTable.objects.prefetch_related('tabledata_set').all()}
...
class TableViewSet(viewsets.ModelViewSet):
    queryset = CharmDateTable.objects.prefetch_related('tabledata_set').all()
...
class TableDataSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            serializer.save()
            return HttpResponseRedirect(redirect_to='http://127.0.0.1:8000/charm-date/')
