...
from anastasiadating.models import AnastasiaDatingTable, TableData
from anastasiadating.serializers import TableSerializer, DataSerializer
from users.models import Client, User
from anastasiadating.forms import CreateAnastasiadatingTable


# Create your views here.


class AnastasiaDatingWorkpage(LoginRequiredMixin,View):
    #permission_required = 'onlyfans.view_onlyfanstable'
    template = 'anastasiadating_template/workpage.html'
    ...
    def post(self,request):
        ...
        table = AnastasiaDatingTable.objects.filter(id=int(table_id))
        new_data = TableData(
...
def get_id_list():
    ...
    data = (int(i.operator.pk) for i in AnastasiaDatingTable.objects.all())
...
def make_get():
    ...
    data = {f'table_{a.id}': {'table':
                 ...
                 for a in
            AnastasiaDatingTable.objects.prefetch_related('tabledata_set').all()}
...
class CreateNewTable(LoginRequiredMixin, View):
    ...
    template = 'anastasiadating_template/create_table.html'

    def get(self, request):
        context = {
            'form': CreateAnastasiadatingTable,
        }
        return render(request,self.template, context)

    def post(self, request):

        form = CreateAnastasiadatingTable(request.POST)
        ...
        if form.is_valid:
            if request.POST['table_type'] == '0':
                new_table = AnastasiaDatingTable(
                    date=month,
                ...
                new_table.save()
        return redirect('anastasiadating_workpage')


class TableViewSet(viewsets.ModelViewSet):
    queryset = AnastasiaDatingTable.objects.prefetch_related('tabledata_set').all()
...
class TableDataSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            serializer.save()
            return HttpResponseRedirect(redirect_to='http://127.0.0.1:8000/anastasiadating/')
